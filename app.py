...
def _effective_cpu_threads() -> int:
    """Choose FFmpeg thread counts from the container CPU quota, not host CPUs.

    Render can expose more logical CPUs than the service is actually entitled to.
    Oversubscribing x264/filter workers on a throttled container can make encoding
    slower due to scheduler contention. An explicit quota-aware value keeps one
    encode aligned with the CPU budget while still allowing parallelism on larger
    instances.
    """
