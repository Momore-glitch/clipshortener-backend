...
VERSION = "30.0.0-release-candidate"
...
def clips_have_even_durations(clips: list[Path], clip_length: float, tolerance: float = 0.25) -> bool:
    ...
    for duration in durations[:-1]:
        if abs(duration - clip_length) > tolerance:
            return False
    if len(durations) == 1:
        return durations[0] <= clip_length + tolerance
...
def create_clips(job_id: str, source: Path, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> list[Path]:
    ...
    if fast_copy:
        ...
        if len(copy_clips) < expected or not clips_have_even_durations(copy_clips, clip_length):
    ...
    if not copy_clips:
        ...
        # Original-format clips are the common fast path. If stream-copy
        # cannot produce all requested segments because of keyframe placement,
        # fall back to a deliberately fast encode. Transcoding is still used
        # for resized formats and caption-burned clips.
        preset = "ultrafast" if export_format == "original" and not captions else "veryfast"
        cmd = [
