...
VERSION = "42.0.0-final-speed-reliability"
...
JOB_TTL = 60 * 60 * 6
MAX_WORKERS = max(1, min(2, int(os.getenv("CLIPSHORTENER_WORKERS", "2"))))  # FIFO active-job pool; one job is fastest alone, two jobs share CPU with priority for the first.
RATE_LIMIT = 20
RATE_WINDOW = 600
...
whisper_model = None
ffmpeg_active: dict[str, float] = {}
ffmpeg_active_lock = threading.Lock()

def _ffmpeg_priority(job_id: str) -> int:
    """Return Linux niceness for active FFmpeg jobs: FIFO first gets highest priority."""
    with ffmpeg_active_lock:
        ffmpeg_active.setdefault(job_id, time.monotonic())
        ordered = sorted(ffmpeg_active.items(), key=lambda item: item[1])
        rank = next((i for i, (jid, _) in enumerate(ordered) if jid == job_id), 0)
    return 0 if rank == 0 else min(10, rank * 5)

def _ffmpeg_done(job_id: str) -> None:
    with ffmpeg_active_lock:
        ffmpeg_active.pop(job_id, None)
...
FFMPEG_THREADS = max(0, min(8, int(os.getenv("CLIPSHORTENER_FFMPEG_THREADS", "0"))))
...
def run_ffmpeg_progress(
    ...
    priority = _ffmpeg_priority(job_id)
    launch_cmd = (["nice", "-n", str(priority)] + full_cmd) if priority > 0 and shutil.which("nice") else full_cmd
    try:
    ...
    return subprocess.CompletedProcess(launch_cmd, returncode, "\n".join(stdout_lines), stderr)
...
def burn_captions(video: Path, ass_path: Path, output: Path, job_id: str | None=None, duration: float | None=None) -> None:
    ...
    cmd=[FFMPEG,"-hide_banner","-loglevel","error","-y","-i",str(video),"-vf",f"subtitles='{escaped}'","-map","0:v:0","-map","0:a?","-c:v","libx264","-preset","ultrafast","-crf","21","-threads",str(FFMPEG_THREADS),"-c:a","aac","-b:a","128k",str(output)]
    ...
    if captions_enabled:
        ...
        try:
            ...
            cmd = [
                ...
                *( ["-c:a", "copy"] if str(info.get("audio_codec", "")).lower() == "aac" else ["-c:a", "aac", "-b:a", "128k"] ), "-pix_fmt", "yuv420p",
                "-f", "segment", "-segment_time", f"{clip_length:.6f}",
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        ...
        "queue_model": "FIFO active-job pool; first active job gets highest FFmpeg OS priority, additional active jobs receive a lower-priority CPU share",
...
def download(job_id: str, name: str):
    path = _safe_clip_path(job_id, name)
    return FileResponse(
        path, media_type="video/mp4", filename=path.name,
...
def download_zip(job_id: str, request: Request):
    ...
    return FileResponse(
        zpath, media_type="application/zip", filename=zpath.name,
        headers={"Cache-Control": "private, max-age=3600", "X-Content-Type-Options": "nosniff"},
    )
