...
VERSION = "40.0.0-fast-caption-range-preview"
...
JOB_TTL = 60 * 60 * 6
MAX_WORKERS = 1  # CPU-bound FFmpeg jobs are serialized for maximum throughput on free-tier CPU; uploads remain concurrent.
RATE_LIMIT = 20
RATE_WINDOW = 600
...
FFMPEG_THREADS = max(0, min(4, int(os.getenv("CLIPSHORTENER_FFMPEG_THREADS", "0"))))
...
def caption_style(style: str, font: str, size: str, position: str, animation: str, canvas_w: int, canvas_h: int) -> dict[str, Any]:
    base = max(82, min(132, round(min(canvas_w, canvas_h) * 0.125)))
    sizes = {"medium": round(base * 0.78), "large": base, "xlarge": round(base * 1.20)}
    pos = {"bottom": (2, max(82, round(canvas_h * 0.10))), "middle": (5, 0), "top": (8, max(82, round(canvas_h * 0.10)))}
    ...
    return {
        ...
        "outline": max(5, round(sizes.get(size, sizes["large"]) * 0.085)),
        "shadow": 3 if not boxed else 0,
        "back": "&HCC000000" if boxed else "&H00000000",
        "border_style": 3 if boxed else 1,
        "align": align,
        "margin_v": margin_v,
        "margin_lr": max(60, round(canvas_w * 0.06)),
...
def _caption_segments(video: Path, language: str, clip_start: float = 0.0, clip_end: float | None = None) -> list[tuple[float, float, str]]:
    """Transcribe only the requested source window and reuse the worker model.

    Explicit English uses the smaller English-only model. For other languages,
    use the multilingual tiny model. The transcription window is limited to
    the actual export range so a long source does not pay to transcribe unused
    audio.
    """
    ...
    # Keep one cached model at a time on the constrained instance. Switching
    # languages is uncommon; if it happens, replace the cached model cleanly.
    if whisper_model is None or getattr(whisper_model, "_clipshortener_model_name", None) != model_key:
        with whisper_model_lock:
            if whisper_model is None or getattr(whisper_model, "_clipshortener_model_name", None) != model_key:
                log.info("Loading faster-whisper %s model (cached worker model)", model_name)
                cache_root = os.getenv("HF_HOME", "/opt/huggingface")
                whisper_model = WhisperModel(
                    model_name, device="cpu", compute_type="int8",
                    download_root=cache_root,
                )
                try:
                    setattr(whisper_model, "_clipshortener_model_name", model_key)
                except Exception:
                    pass

    lang = None if requested_language in {"", "auto"} else requested_language
    kwargs: dict[str, Any] = {
        "beam_size": 1,
        "vad_filter": True,
        "language": lang,
    ...
    if clip_end is not None and clip_end > clip_start + 0.05:
        # faster-whisper accepts [start, end] timestamps and avoids decoding
        # unrelated audio from long source videos.
    ...
    for seg in segments:
        ...
        if text and float(seg.end) > float(seg.start):
            ...
            # Some older backends return timestamps relative to the requested
            # window. Detect that shape and restore source-relative timing.
            if clip_start > 0.05 and en <= (clip_end - clip_start + 0.5 if clip_end else 999999):
                if st < clip_start - 0.5:
                    st += clip_start
                    en += clip_start
...
def create_clips(job_id, source, clip_length, export_format, start, end, captions, info=None):
    ...
    if captions_enabled:
        ...
        try:
            ...
            cmd = [
                ...
                "-crf", "21" if quality == "original" else "23",
    ...
    # ---------------------------------------------------------------
    # 3) ONE-PASS TRANSFORMED EXPORT
    # ---------------------------------------------------------------
    # Any resolution/aspect-ratio transformation is encoded exactly once.
    last_boundary = max(0.0, total_duration - 0.05)
    force_expr = f"expr:if(lt(t,{last_boundary:.3f}),gte(t,n_forced*{clip_length:.6f}),0)"
    audio_args = ["-c:a", "copy"] if str(info.get("audio_codec", "")).lower() == "aac" else ["-c:a", "aac", "-b:a", "128k"]
    cmd = [
        FFMPEG, "-hide_banner", "-loglevel", "error", "-y",
        "-sws_flags", "fast_bilinear",
        "-ss", f"{start:.3f}", "-i", str(source),
        ...
        "-threads", str(FFMPEG_THREADS),
        "-force_key_frames", force_expr,
        *audio_args, "-pix_fmt", "yuv420p", "-movflags", "+faststart",
        "-f", "segment", "-segment_time", f"{clip_length:.6f}",
        "-segment_time_delta", "0.02", "-reset_timestamps", "1",
    ]
    if transform:
    ...
    run_ffmpeg_progress(
        cmd, job_id, total_duration, 20, 55,
        max(900, int(total_duration * 6)),
    )
    clips = sorted(p for p in clip_dir.glob("clip_*.mp4") if p.is_file() and p.stat().st_size > 0)
    if len(clips) > expected:
...
def _zip_lock(key: str) -> threading.Lock:
    with zip_locks_guard:
        return zip_locks.setdefault(key, threading.Lock())


def _build_zip_atomic(zpath: Path, entries: list[tuple[Path, str]]) -> Path:
    if zpath.is_file() and zpath.stat().st_size > 0:
        return zpath
    tmp = zpath.with_name(f".{zpath.name}.{uuid.uuid4().hex}.part")
    try:
        ...
        if tmp.stat().st_size <= 0:
            raise RuntimeError("The ZIP archive could not be created.")
...
def build_zip(job_id: str, clips: list[Path]) -> Path:
    ...
    entries = [(clip, clip.name) for clip in clips]
    with _zip_lock(f"job:{job_id}"):
        return _build_zip_atomic(zpath, entries)
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        ...
        "queue_model": "FIFO CPU scheduler; one encode gets the full constrained CPU at a time, uploads/status remain concurrent",
...
def run_url_import_job(
    ...
    try:
        ...
        # All CPU-heavy work goes through the same FIFO executor. Acquisition
...
def download_zip(job_id: str, request: Request):
    ...
    return _range_response(request, zpath, "application/zip", "attachment", zpath.name)


@app.get("/api/batch-zip")
def download_batch_zip(request: Request, jobs_param: str = Query("", alias="jobs")):
    ...
    for number, job_id in enumerate(ids, 1):
        ...
        for clip in sorted(clip_dir.glob("clip_*.mp4")):
            if clip.is_file() and clip.stat().st_size > 0:
                entries.append((clip, f"video_{number}/{clip.name}"))
    if not entries:
        raise HTTPException(404, "No completed clips were found for this batch.")
    with _zip_lock(f"batch:{','.join(ids)}"):
        zpath = _build_zip_atomic(zpath, entries)
    return _range_response(request, zpath, "application/zip", "attachment", zpath.name)
