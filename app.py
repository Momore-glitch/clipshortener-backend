...
VERSION = "31.0.0-speed-accuracy-fix"
...
def caption_tracks(video: Path, language: str, style: str, font: str, size: str, animation: str, position: str, job_dir: Path) -> tuple[Path, Path]:
    ...
    if whisper_model is None:
        ...
        whisper_model = WhisperModel(WHISPER_MODEL_NAME, device="cpu", compute_type="int8", cpu_threads=1, num_workers=1)
    lang = None if language in {"", "auto", None} else language
    segments, _ = whisper_model.transcribe(str(video), beam_size=1, best_of=1, temperature=0, vad_filter=True, language=lang or (WHISPER_LANGUAGE or None), condition_on_previous_text=False)
...
def burn_captions(video: Path, ass_path: Path, output: Path) -> None:
    ...
    run_checked([
        ...
        "-c:v", "libx264", "-preset", "ultrafast", "-crf", "23", "-threads", str(FFMPEG_THREADS), "-c:a", "aac", "-b:a", "128k", "-movflags", "+faststart", str(output),
    ], timeout=1800)
...
def create_clips(job_id: str, source: Path, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> list[Path]:
    ...
    # Fastest possible path: remux/segment without transcoding.  break_non_keyframes
    # lets the segmenter cut at the requested times even when the source has sparse
    # keyframes.  We validate every result; if a container/codec refuses accurate
    # boundaries we fall back to one-pass ultrafast encoding.
    ...
    if fast_copy:
        ...
        if len(copy_clips) != expected or not clips_have_even_durations(copy_clips, clip_length, tolerance=0.30):
    ...
    # All non-original outputs necessarily require a transcode. Keep those encodes
    # as cheap as possible while preserving the requested dimensions.
    preset = "ultrafast"
    cmd = [
        ...
        "-force_key_frames", f"expr:gte(t,n_forced*{clip_length})",
        "-c:a", "aac", "-b:a", "128k", "-f", "segment", "-segment_time", f"{clip_length:.3f}",
    ...
    if captions and captions.get("enabled"):
        caption_assets: list[tuple[Path, Path, Path]] = []
        try:
            for index, clip in enumerate(clips, 1):
                update_job(job_id, progress=int(55 + 15 * index / max(1, len(clips))), message=f"Transcribing captions: clip {index} of {len(clips)}")
                ...
                caption_assets.append((clip, ass, vtt))
        finally:
            # Do not keep the speech model resident while FFmpeg burns captions.
            global whisper_model
            whisper_model = None
            try:
                import gc
                ...
                pass
        for index, (clip, ass, vtt) in enumerate(caption_assets, 1):
            update_job(job_id, progress=int(70 + 15 * index / max(1, len(caption_assets))), message=f"Burning captions: clip {index} of {len(caption_assets)}")
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        ...
        "release_status": "speed-accuracy-fix",
    }
