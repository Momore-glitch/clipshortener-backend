...
VERSION = "39.0.1-fifo-fast-media-preview"
...
def _caption_segments(video: Path, language: str) -> list[tuple[float, float, str]]:
    ...
    if whisper_model is None:
        with whisper_model_lock:
            if whisper_model is None:
                log.info("Loading faster-whisper tiny model (cached worker model)")
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        ...
        "queue_model": "FIFO single CPU worker; first queued job receives the available FFmpeg CPU, then the next job",
