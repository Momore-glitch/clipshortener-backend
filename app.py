...
# YouTube links are handled through LumiClip rather than attempting to bypass
# YouTube authentication/bot protections locally. The API key stays server-side.
LUMICLIP_API_KEY = os.getenv("LUMICLIP_API_KEY", "").strip()
LUMICLIP_API_URL = os.getenv("LUMICLIP_API_URL", "https://api.lumiclip.ai").rstrip("/")
LUMICLIP_POLL_SECONDS = max(2, float(os.getenv("LUMICLIP_POLL_SECONDS", "4")))
LUMICLIP_MAX_POLL_SECONDS = max(60, int(os.getenv("LUMICLIP_MAX_POLL_SECONDS", "900")))
...
def _youtube_auth_required(*errors: object) -> bool:
    text = " ".join(str(x or "") for x in errors).lower()
...
def _lumiclip_headers() -> dict[str, str]:
    if not LUMICLIP_API_KEY:
        raise RuntimeError("LumiClip is not configured on the server. Set LUMICLIP_API_KEY in Render.")
    return {
        "Authorization": f"Bearer {LUMICLIP_API_KEY}",
        "Accept": "application/json",
        "Content-Type": "application/json",
        "User-Agent": "ClipShortener/20.1",
    }


def _lumiclip_json(method: str, path: str, body: dict[str, Any] | None = None, timeout: int = 60) -> dict[str, Any]:
    url = f"{LUMICLIP_API_URL}{path}"
    data = json.dumps(body).encode("utf-8") if body is not None else None
    req = URLRequest(url, data=data, headers=_lumiclip_headers(), method=method.upper())
    try:
        with urlopen(req, timeout=timeout) as response:
            raw = response.read().decode("utf-8", errors="replace")
        ...
        raise RuntimeError(f"LumiClip returned HTTP {exc.code}: {detail}") from exc
    except (URLError, TimeoutError, OSError) as exc:
        raise RuntimeError(f"Could not connect to LumiClip: {exc}") from exc
    if not 200 <= status < 300:
        raise RuntimeError(f"LumiClip returned HTTP {status}: {raw[:1200]}")
    try:
        data_out = json.loads(raw or "{}")
    except json.JSONDecodeError as exc:
        raise RuntimeError("LumiClip returned invalid JSON.") from exc
    if not isinstance(data_out, dict):
        raise RuntimeError("LumiClip returned an unexpected response.")
    return data_out


def _canonicalize_youtube_url(url: str) -> str:
    """Convert common YouTube URL variants to a canonical watch URL.

    This does not bypass authentication or access controls; it only normalizes
    the URL so providers receive a standard YouTube watch URL.
    """
...
def _lumiclip_generate(url: str, start: float, end: float | None) -> str:
    # LumiClip documents YouTube URL ingestion. Normalize shortened/mobile/
    # shorts URLs to a standard watch URL before sending them to the API.
    # This is only URL normalization; it does not bypass authentication or
    # any YouTube access controls.
    url = _canonicalize_youtube_url(url)
    log.info("LUMICLIP_GENERATE_URL %s", url)
    body: dict[str, Any] = {"url": url}
    if start > 0:
        body["start_time"] = start
    if end is not None and end > start:
        body["end_time"] = end
    data = _lumiclip_json("POST", "/api/v1/clips/generate", body, timeout=90)
    project_id = data.get("project_id") or data.get("id")
    if not project_id:
        raise RuntimeError(f"LumiClip did not return a project ID: {str(data)[:600]}")
    return str(project_id)


def _lumiclip_project(project_id: str) -> dict[str, Any]:
    return _lumiclip_json("GET", f"/api/v1/projects/{project_id}", timeout=60)


def _download_lumiclip_clip(job_id: str, clip: dict[str, Any], index: int) -> Path:
    media_url = str(clip.get("download_url") or "").strip()
    if not media_url:
        clip_id = str(clip.get("id") or "").strip()
        if clip_id:
            # Export first, then use the documented download endpoint.
            _lumiclip_json("POST", f"/api/v1/clips/{clip_id}/export", {"quality": "720p"}, timeout=90)
            media_url = f"{LUMICLIP_API_URL}/api/v1/clips/{clip_id}/download"
    if not media_url:
        raise RuntimeError("LumiClip returned a clip without a download URL.")
    parsed = urlparse(media_url)
    # Documented API download endpoints may require the Bearer header, while
    # returned CDN URLs normally do not. Handle both safely.
    if parsed.hostname and parsed.hostname.lower() == urlparse(LUMICLIP_API_URL).hostname.lower():
        req = URLRequest(media_url, headers=_lumiclip_headers(), method="GET")
        try:
            with urlopen(req, timeout=120) as response, (JOBS_ROOT / job_id / "clips" / f"clip-{index:02d}.mp4").open("wb") as target:
                total = 0
                while True:
                    chunk = response.read(4 * 1024 * 1024)
                    if not chunk: break
                    total += len(chunk)
                    if total > MAX_VIDEO_SIZE: raise RuntimeError("LumiClip clip exceeds the configured size limit.")
                    target.write(chunk)
        except HTTPError as exc:
            detail = exc.read().decode("utf-8", errors="replace")[:1000]
            raise RuntimeError(f"LumiClip download returned HTTP {exc.code}: {detail}") from exc
        path = JOBS_ROOT / job_id / "clips" / f"clip-{index:02d}.mp4"
        if not path.is_file() or path.stat().st_size <= 0:
            raise RuntimeError("LumiClip returned an empty clip.")
        return path
    path = JOBS_ROOT / job_id / "clips" / f"clip-{index:02d}.mp4"
    _download_stream(media_url, path)
    return path


def _download_lumiclip_thumbnail(job_id: str, url: str) -> Path | None:
    if not url:
        return None
    path = JOBS_ROOT / job_id / "source-thumb.jpg"
    try:
        _download_stream(url, path, max_bytes=15 * 1024 * 1024)
        return path
    except Exception as exc:
        log.warning("LUMICLIP_THUMBNAIL_FAILED job=%s error=%s", job_id, exc)
        path.unlink(missing_ok=True)
        return None


def complete_lumiclip_job(job_id: str, url: str, start: float, end: float | None) -> None:
    job_dir = JOBS_ROOT / job_id
    try:
        update_job(job_id, status="processing", progress=8, message="Sending your YouTube video to LumiClip...")
        project_id = _lumiclip_generate(url, start, end)
        update_job(job_id, progress=15, message="LumiClip is processing the video...")
        deadline = time.time() + LUMICLIP_MAX_POLL_SECONDS
        project: dict[str, Any] = {}
        while time.time() < deadline:
            project = _lumiclip_project(project_id)
            status = str(project.get("status") or project.get("project_status") or "").lower()
            if status in {"completed", "complete", "done", "success", "failed", "error", "cancelled"}:
                if status in {"failed", "error", "cancelled"}:
                    raise RuntimeError(f"LumiClip processing failed: {project.get('error') or project.get('message') or status}")
                break
            clips_now = project.get("clips") or []
            if clips_now:
                ready = [c for c in clips_now if str(c.get("clip_status") or c.get("status") or "").lower() in {"completed", "complete", "ready", "done", "success"} or c.get("download_url")]
                if ready and status in {"processing", "generating", "queued", ""}:
                    # Keep polling briefly; the project endpoint is authoritative.
                    pass
            elapsed = max(0, int(time.time() - (deadline - LUMICLIP_MAX_POLL_SECONDS)))
            update_job(job_id, progress=min(85, 15 + int(elapsed / max(1, LUMICLIP_MAX_POLL_SECONDS) * 70)), message="LumiClip is creating your clips...")
            time.sleep(LUMICLIP_POLL_SECONDS)
        else:
            raise RuntimeError("LumiClip processing timed out. Please try again with a shorter source range.")

        clips_data = project.get("clips") or []
        if not isinstance(clips_data, list):
            raise RuntimeError("LumiClip returned an invalid clips list.")
        ready = [c for c in clips_data if isinstance(c, dict) and (c.get("download_url") or c.get("id"))]
        if not ready:
            raise RuntimeError("LumiClip completed but returned no downloadable clips.")
        # Limit to a reasonable batch size so one request cannot fill the disk.
        ready = ready[:20]
        clips_dir = job_dir / "clips"
        clips_dir.mkdir(parents=True, exist_ok=True)
        saved: list[Path] = []
        for i, clip in enumerate(ready, 1):
            update_job(job_id, progress=min(95, 85 + int((i - 1) / max(1, len(ready)) * 10)), message=f"Downloading clip {i} of {len(ready)}...")
            saved.append(_download_lumiclip_clip(job_id, clip, i))

        # LumiClip supplies the actual clip durations. For source metadata, use
        # the project/source duration when present, otherwise sum clip duration.
        duration = float(project.get("duration") or project.get("source_duration") or 0)
        if duration <= 0:
            duration = sum(float(c.get("duration") or 0) for c in ready)
        total_size = sum(p.stat().st_size for p in saved)
        source_info = {"duration": duration, "size": total_size}
        thumb_url = str(project.get("thumbnail_url") or "").strip()
        thumb = _download_lumiclip_thumbnail(job_id, thumb_url) or saved[0]
        if thumb == saved[0]:
            make_thumbnail(saved[0], job_dir / "source-thumb.jpg")
            thumb = job_dir / "source-thumb.jpg"
        zip_path = build_zip(job_id, saved)
        result = render_result(job_id, source_info, "youtube-lumiclip.mp4", saved, thumb, zip_path)
        update_job(job_id, status="complete", progress=100, message="Your LumiClip clips are ready.", result=result)
    except Exception as exc:
        log.exception("LUMICLIP_JOB_FAILED id=%s", job_id)
        update_job(job_id, status="error", progress=0, message=str(exc)[:700], error_code="lumiclip_failed", action="upload")
...
def download_direct_media(job_id: str, url: str) -> tuple[Path, str]:
    ...
    if _host_is_youtube(urlparse(url).hostname or ""):
        raise RuntimeError("YouTube links are handled through LumiClip in the /api/link workflow.")
    dest = JOBS_ROOT / job_id / "source"
    dest.mkdir(parents=True, exist_ok=True)
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        "acquisition": "LumiClip for YouTube + direct-media for authorized direct URLs",
        "youtube_provider": "LumiClip API",
        "lumiclip_api": LUMICLIP_API_URL,
        "lumiclip_configured": bool(LUMICLIP_API_KEY),
        "cobalt_api": COBALT_API_URL,
        "acquisition_mode": "lumiclip-youtube",
...
def acquisition_health() -> dict[str, Any]:
    ...
    lumiclip_configured = bool(LUMICLIP_API_KEY)
    return {
        "ok": lumiclip_configured,
        "lumiclip_configured": lumiclip_configured,
        "cobalt": cobalt_ok,
...
async def link(request: Request, payload: LinkRequest) -> dict[str, Any]:
    ...
    def task() -> None:
        try:
            if _host_is_youtube(urlparse(url).hostname or ""):
                if not LUMICLIP_API_KEY:
                    raise RuntimeError("YouTube clipping is not configured. Set LUMICLIP_API_KEY in the Render environment.")
                complete_lumiclip_job(job_id, url, start_v, end_v)
                return
            ...
            update_job(
                ...
                status="error",
                progress=0,
                message="We couldn't import this video automatically. Please upload the video file instead.",
...
async def source_check(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if _host_is_youtube(host):
        ...
        return {"ok": True, "kind": "youtube", "message": "YouTube link accepted. ClipShortener will send the link to the configured LumiClip provider."}
    if _host_is_platform(host):
