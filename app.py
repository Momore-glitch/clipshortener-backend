...
VERSION = "32.0.0-final-speed-captions-batch-fix"
...
def clips_have_even_durations(clips: list[Path], clip_length: float, tolerance: float = 0.25) -> bool:
    """Verify every non-final segment is at the requested duration.

    The final segment is allowed to be shorter because it is the natural
    remainder of the source range.
    """
    if not clips:
        return False
...
def create_clips(job_id: str, source: Path, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> list[Path]:
    ...
    # Zero-transcode path. It is accepted only when ffprobe confirms that every
    # non-final clip is at the requested duration. Otherwise we re-encode.
    fast_copy = export_format == "original" and not captions and abs(start) <= 0.001 and abs(end_value - duration) <= 0.2
    if fast_copy:
        run_checked([
            FFMPEG, "-hide_banner", "-loglevel", "error", "-y", "-i", str(source),
            "-map", "0:v:0", "-map", "0:a?", "-c", "copy",
            "-f", "segment", "-segment_time", f"{clip_length:.3f}",
            "-break_non_keyframes", "1", "-reset_timestamps", "1", str(pattern),
        ], timeout=max(600, int(duration * 1.5)))
        copy_clips = sorted(p for p in clip_dir.glob("clip_*.mp4") if p.is_file() and p.stat().st_size > 0)
        if len(copy_clips) == expected and clips_have_even_durations(copy_clips, clip_length, tolerance=0.30):
            return copy_clips
        for old in copy_clips:
            old.unlink(missing_ok=True)

    filters = video_filter(export_format)
    # Reliable exact segmentation: force keyframes at each requested boundary,
    # but never at the final timestamp. That prevents FFmpeg creating a zero-byte
    # remainder clip when the source duration is exactly divisible by clip_length.
    ...
    cmd = [
        ...
        "-crf", "23" if export_format != "original" else "21", "-threads", str(FFMPEG_THREADS),
        "-force_key_frames", force_expr,
        "-c:a", "aac", "-b:a", "128k", "-pix_fmt", "yuv420p",
        "-f", "segment", "-segment_time", f"{clip_length:.3f}", "-segment_time_delta", "0.05",
        "-reset_timestamps", "1",
    ]
    if filters:
        cmd += ["-vf", filters]
    cmd += [str(pattern)]
    run_ffmpeg_progress(
        cmd, job_id=job_id, total_seconds=total_duration,
        progress_start=15, progress_span=60,
        timeout=max(900, int(total_duration * 4)),
    )

    clips = sorted(p for p in clip_dir.glob("clip_*.mp4") if p.is_file() and p.stat().st_size > 0)
    # Remove accidental sub-second terminal artifacts; they are not useful clips.
    ...
    if not clips_have_even_durations(clips, clip_length, tolerance=0.30):
    ...
    if captions and captions.get("enabled"):
        # Transcribe the source once. Splitting the transcript into clip-local
        # caption tracks is dramatically faster than running Whisper per clip.
        update_job(job_id, progress=76, message="Transcribing captions once for the full video...")
        try:
            ...
            for index, clip in enumerate(clips, 1):
                ...
                clip_end = min(end_value, clip_start + clip_info["duration"])
                ass = job_dir / f"captions_{index:03d}.ass"
                vtt = job_dir / f"clip_{index:03d}.vtt"
                caption_count = write_caption_files(
                ...
                if caption_count:
                    update_job(job_id, progress=76 + int(18 * index / max(1, len(clips))), message=f"Burning captions: clip {index} of {len(clips)}")
                    temp = clip.with_name(clip.stem + "_cap.mp4")
                    burn_captions(clip, ass, temp)
                    temp.replace(clip)
                    vtt.replace(job_dir / f"{clip.stem}.vtt")
                    ass.unlink(missing_ok=True)
                else:
                    update_job(job_id, progress=76 + int(18 * index / max(1, len(clips))), message=f"No speech in clip {index}; captions skipped.")
        finally:
            global whisper_model
            whisper_model = None
            try:
        ...
        if not clips_have_even_durations(clips, clip_length, tolerance=0.35):
            raise RuntimeError("Caption rendering changed clip timing unexpectedly.")
    return clips
