...
VERSION = "26.2.0-youtube-music-routing"
...
UPLOAD_CHUNK_SIZE = int(os.getenv("CLIPSHORTENER_UPLOAD_CHUNK_MB", "1")) * 1024 * 1024
MAX_BATCH_FILES = 8
...
COBALT_TIMEOUT = int(os.getenv("CLIPSHORTENER_COBALT_TIMEOUT", "120"))
...
def _cobalt_youtube_url(url: str) -> str:
    """Route YouTube video URLs through the YouTube Music hostname for Cobalt.

    Cobalt/YouTube integrations have historically been sensitive to the regular
    YouTube web hostname. Keep the user-facing URL unchanged while sending the
    equivalent single-video Music URL to the acquisition service.
    """
    parsed = urlparse(url)
    ...
    # Preserve a user-specified start position, but intentionally do not carry
    # playlist/session parameters into the single-video acquisition request.
    keep = []
    query = dict(parse_qsl(parsed.query, keep_blank_values=True))
    if "t" in query:
        keep.append(("t", query["t"]))
    music_query = [("v", video_id), *keep]
    return "https://music.youtube.com/watch?" + urlencode(music_query)


def _cobalt_request(url: str) -> dict[str, Any]:
    if not COBALT_API_URL:
        raise RuntimeError("Platform links are not configured yet. Set COBALT_API_URL on the processing server.")
    endpoint = COBALT_API_URL + "/"
    cobalt_url = _cobalt_youtube_url(url)
    body = {
        "url": cobalt_url,
        "downloadMode": "auto",
        "videoQuality": "max",
    ...
    try:
        ...
        try:
            ...
            exc.close()
        raise RuntimeError(f"Platform acquisition service returned HTTP {exc.code}: {detail[:300]}") from exc
    except (URLError, TimeoutError, OSError) as exc:
        raise RuntimeError("Could not reach the platform acquisition service.") from exc
    try:
        data = json.loads(raw.decode("utf-8"))
    except Exception as exc:
        raise RuntimeError(f"Platform acquisition service returned invalid JSON (HTTP {status}).") from exc
    if not isinstance(data, dict):
        raise RuntimeError("Platform acquisition service returned an invalid response.")
    if cobalt_url != url:
        log.info("COBALT_YOUTUBE_MUSIC_ROUTE source=%s target=%s", url, cobalt_url)
    return data


def _acquire_cobalt_video(job_id: str, url: str) -> tuple[Path, str]:
    parsed = urlparse(url)
    if not _host_is_platform(parsed.hostname or ""):
        raise RuntimeError("This source is not a supported platform URL.")
    result = _cobalt_request(url)
    status = result.get("status")
    if status == "picker":
        items = [item for item in result.get("picker", []) if isinstance(item, dict) and item.get("type") == "video" and item.get("url")]
        if not items:
            raise RuntimeError("The platform returned no downloadable video item.")
        download_url = items[0]["url"]
        filename = safe_filename("platform-video.mp4")
    elif status in {"tunnel", "redirect"} and result.get("url"):
        download_url = str(result["url"])
        filename = safe_filename(str(result.get("filename") or "platform-video.mp4"))
    else:
        error = result.get("error") if isinstance(result.get("error"), dict) else {}
        code = error.get("code") or "unknown"
        raise RuntimeError(f"Platform acquisition failed: {code}")
    ...
    # Older/misconfigured Cobalt instances may advertise a loopback tunnel URL.
    # Since the backend already knows the public Cobalt endpoint, safely rewrite
    # that local origin while preserving the returned tunnel path/query.
    host = (download_parsed.hostname or "").lower()
    if host in {"127.0.0.1", "localhost", "::1"} and COBALT_API_URL:
        base = urlparse(COBALT_API_URL)
        download_url = urljoin(COBALT_API_URL + "/", download_parsed.path.lstrip("/") + ("?" + download_parsed.query if download_parsed.query else ""))
        download_parsed = urlparse(download_url)
    if not _host_is_public(download_parsed.hostname or ""):
    ...
    req = URLRequest(download_url, headers={"User-Agent": "ClipShortener/26.0", "Accept": "video/*,application/octet-stream;q=0.9,*/*;q=0.1"})
    try:
        with urlopen(req, timeout=COBALT_TIMEOUT) as response:
        ...
        source.unlink(missing_ok=True)
        raise RuntimeError("Could not download the acquired platform video.") from exc
    if total <= 0:
        source.unlink(missing_ok=True)
        raise RuntimeError("The platform returned an empty video file.")
    log.info("PLATFORM_IMPORT_SUCCESS job=%s service_host=%s size=%s filename=%s", job_id, parsed.hostname, total, filename)
...
def _acquire_video(job_id: str, url: str) -> tuple[Path, str]:
    ...
    if is_platform:
        return _acquire_cobalt_video(job_id, value)
...
def _acquire_public_video(job_id: str, url: str) -> tuple[Path, str]:
    ...
    for _ in range(MAX_URL_REDIRECTS + 1):
        ...
        req = URLRequest(current, headers={"User-Agent": "ClipShortener/25.0", "Accept": "video/*,application/octet-stream;q=0.9,*/*;q=0.1"})
        try:
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        ...
        "input_mode": "uploaded files + direct public video URLs + supported platform URLs",
        "platform_acquisition": "configured" if COBALT_API_URL else "not_configured",
...
def download_batch_zip(jobs_param: str = "") -> FileResponse:
