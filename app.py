...
def _target_dimensions(info: dict[str, Any], export_format: str) -> tuple[int, int] | None:
    ...
    if quality == "original" and ratio == "source":
        return None
    if quality == "original":
        # Keep the source pixel dimensions and crop only the excess needed for
        # the requested aspect ratio. Never upscale an original-quality export.
        aspect = sw / sh
        if ratio == "16:9":
            if aspect > 16 / 9:
                tw, th = int(sh * 16 / 9), sh
            else:
                tw, th = sw, int(sw * 9 / 16)
        elif ratio == "9:16":
            if aspect > 9 / 16:
                tw, th = int(sh * 9 / 16), sh
            else:
                tw, th = sw, int(sw * 16 / 9)
        else:
            side = min(sw, sh); tw = th = side
        return (max(2, tw // 2 * 2), max(2, th // 2 * 2))
    if ratio == "source":
        if quality == "720":
            scale = min(1280 / sw, 720 / sh, 1.0)
        else:
            scale = min(1920 / sw, 1080 / sh, 1.0)
        if scale >= 0.999:
            return (sw - sw % 2, sh - sh % 2)
        return (max(2, int(sw * scale) // 2 * 2), max(2, int(sh * scale) // 2 * 2))
    if quality == "720": base = 720
    elif quality == "1080": base = 1080
    else: return None
    if ratio == "16:9": return (1280, 720) if base == 720 else (1920, 1080)
    if ratio == "9:16": return (720, 1280) if base == 720 else (1080, 1920)
    return (720, 720) if base == 720 else (1080, 1080)
...
def video_filter(export_format: str, info: dict[str, Any] | None = None) -> str | None:
    ...
    if target is None:
        return None
    tw, th = target
    if (tw, th) == (sw - sw % 2, sh - sh % 2):
        return None
    if ratio == "source":
        if (sw, sh) == (tw, th): return None
        return f"scale={tw}:{th}:flags=fast_bilinear"
    # Fill the requested canvas then crop the excess. This makes the selected
    # ratio real instead of padding the video with bars.
    return f"scale={tw}:{th}:force_original_aspect_ratio=increase:flags=fast_bilinear,crop={tw}:{th}:(iw-{tw})/2:(ih-{th})/2"
...
def caption_style(style: str, font: str, size: str, position: str, animation: str, canvas_w: int, canvas_h: int) -> dict[str, Any]:
    base = max(48, min(96, round(min(canvas_w, canvas_h) * 0.065)))
...
def _caption_segments(video: Path, language: str) -> list[tuple[float, float, str]]:
    ...
    segments, _ = whisper_model.transcribe(str(video), beam_size=1, vad_filter=True, language=lang, condition_on_previous_text=False)
    out: list[tuple[float, float, str]] = []
    for seg in segments:
        text = " ".join((seg.text or "").strip().split())
        if text and float(seg.end) > float(seg.start): out.append((float(seg.start), float(seg.end), text))
    if not out: raise RuntimeError("No speech was detected for automatic captions.")
    return out
...
def burn_captions(video: Path, ass_path: Path, output: Path, job_id: str | None=None, duration: float | None=None) -> None:
    ...
    cmd=[FFMPEG,"-hide_banner","-loglevel","error","-y","-i",str(video),"-vf",f"subtitles='{escaped}'","-map","0:v:0","-map","0:a?","-c:v","libx264","-preset","ultrafast","-crf","21","-threads",str(FFMPEG_THREADS),"-c:a","aac","-b:a","128k","-movflags","+faststart",str(output)]
...
def create_clips(job_id, source, clip_length, export_format, start, end, captions, info=None):
    info=info or probe(source); duration=info["duration"]
    start=min(max(0.0,start),max(0.0,duration-0.05)); end_value=duration if end is None else min(max(start+0.05,end),duration)
    total_duration=max(0.05,end_value-start); expected=max(1,math.ceil((total_duration-1e-9)/clip_length))
    job_dir=JOBS_ROOT/job_id; clip_dir=job_dir/"clips"; clip_dir.mkdir(exist_ok=True)
    ...
    copy_allowed=(transform is None) and not captions
    if copy_allowed:
        copy_cmd=[FFMPEG,"-hide_banner","-loglevel","error","-y"]
        if start>0.001: copy_cmd += ["-ss",f"{start:.3f}"]
        ...
        try:
            ...
            if _durations_are_consistent(copy_clips,clip_length,total_duration): return copy_clips
            for old in copy_clips: old.unlink(missing_ok=True)
        except Exception: 
            for old in clip_dir.glob("clip_*.mp4"): old.unlink(missing_ok=True)
    cmd=[FFMPEG,"-hide_banner","-loglevel","error","-y","-sws_flags","fast_bilinear","-ss",f"{start:.3f}","-i",str(source),"-t",f"{total_duration:.3f}","-map","0:v:0","-map","0:a?","-c:v","libx264","-preset","ultrafast","-tune","zerolatency","-crf","21","-threads",str(FFMPEG_THREADS),"-force_key_frames",f"expr:if(lt(t,{max(0,total_duration-0.05):.3f}),gte(t,n_forced*{clip_length:.6f}),0)","-c:a","aac","-b:a","128k","-f","segment","-segment_time",f"{clip_length:.6f}","-segment_time_delta","0.02","-reset_timestamps","1"]
    if transform: cmd += ["-vf",transform]
    cmd += [str(pattern)]
    ...
    if len(clips)>expected:
        for extra in clips[expected:]: extra.unlink(missing_ok=True)
        clips=clips[:expected]
    if not clips or len(clips)!=expected or not _durations_are_consistent(clips,clip_length,total_duration,tolerance=0.30):
    ...
    if captions and captions.get("enabled"):
        ...
        target=_target_dimensions(info,export_format) or (int(info.get("width") or 1920),int(info.get("height") or 1080))
        cw,ch=target
        for index,clip in enumerate(clips,1):
            clip_start=start+(index-1)*clip_length
            clip_duration = clip_length if index < len(clips) else max(0.001, total_duration - clip_length * (len(clips)-1))
            ass=job_dir/f"captions_{index:03d}.ass"; vtt=job_dir/f"captions_{index:03d}.vtt"
            caption_tracks_from_segments(segments,clip_start,clip_duration,captions.get("style","classic"),captions.get("font","DejaVu Sans"),captions.get("size","large"),captions.get("animation","none"),captions.get("position","bottom"),ass,vtt,cw,ch)
            if ass.exists():
                temp=clip.with_name(clip.stem+"_cap.mp4"); burn_captions(clip,ass,temp,job_id,clip_duration); temp.replace(clip)
            if vtt.exists(): vtt.rename(job_dir/f"{clip.stem}.vtt")
...
def render_result(job_id: str, source_info: dict[str, Any], source_name: str, clips: list[Path], thumb: Path, zip_path: Path, clip_length: float, start: float, total_duration: float) -> dict[str, Any]:
    ...
    return {"success":True,"job_id":job_id,"source_name":source_name,"source_duration":round(source_info["duration"],2),"source_size":source_info["size"],"clip_count":len(items),"clips":items,"zip":f"/api/jobs/{job_id}/zip","thumbnail":f"/api/jobs/{job_id}/source-thumb"}
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        ...
        "queue_model": "bounded single-worker processing; uploads remain concurrent",
