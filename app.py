...
def _target_dimensions(info: dict[str, Any], export_format: str) -> tuple[int, int] | None:
    ...
    # Never upscale a source merely because the user selected a larger export
    # preset. Upscaling 720p material to 1080x1920 can more than double the
    # pixels the encoder must process without adding real detail. The export
    # remains "up to" the selected quality, which is both faster and visually
    # more faithful to the source.
    source_long_edge = max(sw, sh)
    if NO_UPSCALE and base == 1080 and source_long_edge < 1000:
        base = 720
