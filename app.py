...
VERSION = "33.2.0-caption-canvas-speed-fix"
...
JOB_TTL = 60 * 60 * 6
# Never expire a job solely because its directory was created long ago; use the
# persisted job-state timestamp so active/recovered jobs remain available.
...
whisper_model = None
WHISPER_MODEL_NAME = os.getenv("CLIPSHORTENER_WHISPER_MODEL", "tiny").strip() or "tiny"
WHISPER_LANGUAGE = os.getenv("CLIPSHORTENER_WHISPER_LANGUAGE", "").strip()
...
def _acquire_video(job_id: str, url: str) -> tuple[Path, str]:
    ...
    if is_platform:
        raise RuntimeError(
            "Platform-page links are not supported. Paste a direct public video-file URL instead."
        )
...
def cleanup_old_jobs() -> None:
    ...
    for path in list(JOBS_ROOT.iterdir()):
        try:
            ...
            try:
                updated = float(json.loads(state_path.read_text(encoding="utf-8")).get("updated_at", path.stat().st_mtime)) if state_path.is_file() else path.stat().st_mtime
            except Exception:
    ...
    for key, value in list(jobs.items()):
        if float(value.get("updated_at", now()) or now()) < cutoff:
            jobs.pop(key, None)
                ...
                try:
                    ...
                    if current != last_progress:
                        ...
                        update_job(
                            job_id,
                            progress=current,
                            message=f"Encoding clips... {current}%",
                        )
...
EXPORT_FORMATS = {"original", "720p", "1080p", "vertical", "square", "landscape"}
...
def validate_options(clip_length: Any, export_format: str, start: Any, end: Any) -> tuple[float, str, float, float | None]:
    ...
    export_value = str(export_format or "original").strip().lower()
    if export_value not in EXPORT_FORMATS:
        raise HTTPException(400, f"Unsupported export format: {export_value}.")
    export = export_value
...
def _caption_settings(style: str, font: str, size: str, position: str, animation: str) -> dict[str, Any]:
    sizes = {"medium": 46, "large": 58, "xlarge": 70}
    return {
        "font": font if font in {"DejaVu Sans", "Liberation Sans", "DejaVu Sans Mono"} else "DejaVu Sans",
        "size": sizes.get(size, 58),
...
def _vtt_time(seconds: float) -> str:
    ...
    sec = int(total); ms = int(round((total - sec) * 1000))
    if ms >= 1000:
        sec += 1; ms -= 1000
    return f"{h:02d}:{m:02d}:{sec:02d}.{ms:03d}"


def transcribe_source(video: Path, language: str) -> list[dict[str, Any]]:
    """Transcribe once per source, rather than once per output clip."""
    global whisper_model
    if WhisperModel is None:
        raise RuntimeError("Automatic captions are not installed on this processing server. Rebuild the Render service with the current requirements.txt and Dockerfile.")
    if whisper_model is None:
        log.info("Loading faster-whisper %s", WHISPER_MODEL_NAME)
        whisper_model = WhisperModel(
            ...
            cpu_threads=1,
            num_workers=1,
            download_root=os.getenv("CLIPSHORTENER_WHISPER_CACHE", "/opt/huggingface"),
        )
    ...
    segments, _ = whisper_model.transcribe(
        ...
        temperature=0,
        vad_filter=True,
        language=lang or (WHISPER_LANGUAGE or None),
    ...
    for seg in segments:
        ...
        if text and float(seg.end) > float(seg.start):
            result.append({"start": float(seg.start), "end": float(seg.end), "text": text})
    # Silence is a valid video state: export the clips normally and simply omit
    # captions where no speech exists. Model/runtime failures still raise above.
    return result


def caption_canvas(export_format: str, source_width: int, source_height: int) -> tuple[int, int]:
    if export_format in {"720p", "landscape"}: return 1280, 720
    if export_format == "1080p": return 1920, 1080
    if export_format == "vertical": return 720, 1280
    if export_format == "square": return 720, 720
    return max(2, source_width or 1920), max(2, source_height or 1080)
...
def write_caption_files(
    ...
    settings = _caption_settings(style, font, size, position, animation)
    count = 0
    with ass_path.open("w", encoding="utf-8") as af, vtt_path.open("w", encoding="utf-8") as vfout:
        ...
        vfout.write("WEBVTT\n\n")
        for seg in segments:
            if float(seg["end"]) <= clip_start or float(seg["start"]) >= clip_end:
                continue
            rel_start = max(0.0, float(seg["start"]) - clip_start)
            rel_end = min(clip_end, float(seg["end"])) - clip_start
            if rel_end <= rel_start + 0.01:
                continue
            text = ass_escape(seg["text"])
            af.write(f"Dialogue: 0,{_ass_time(rel_start)},{_ass_time(rel_end)},Default,,0,0,0,,{{{settings['tags']}}}{text}\n")
            vfout.write(f"{_vtt_time(rel_start)} --> {_vtt_time(rel_end)}\n{seg['text']}\n\n")
            count += 1
    if count == 0:
...
def burn_captions(video: Path, ass_path: Path, output: Path) -> None:
    run_checked([
        ...
        "-vf", _subtitle_filter(ass_path), "-map", "0:v:0", "-map", "0:a?",
        "-c:v", "libx264", "-preset", "ultrafast", "-crf", "23", "-threads", str(FFMPEG_THREADS),
        "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "128k", "-movflags", "+faststart", str(output),
    ], timeout=1800)


def _burn_captioned_source(
    source: Path,
    ass_path: Path,
    output: Path,
    start: float,
    ...
    filters: list[str] = []
    export_filter = video_filter(export_format)
    if export_filter:
        filters.append(export_filter)
    filters.append(_subtitle_filter(ass_path))
    last_boundary = max(0.0, total_duration - 0.05)
    force_expr = f"expr:if(lt(t,{last_boundary:.3f}),gte(t,n_forced*{clip_length:.3f}),0)"
    cmd = [
        FFMPEG, "-hide_banner", "-loglevel", "error", "-y",
        "-ss", f"{start:.3f}", "-i", str(source), "-t", f"{total_duration:.3f}",
    ...
    run_ffmpeg_progress(cmd, job_id=job_id, total_seconds=total_duration, progress_start=15, progress_span=60,
                        timeout=max(900, int(total_duration * 4)))


def _segment_copy(video: Path, pattern: Path, clip_length: float) -> list[Path]:
    run_checked([
        FFMPEG, "-hide_banner", "-loglevel", "error", "-y", "-i", str(video),
        "-map", "0:v:0", "-map", "0:a?", "-c", "copy",
        "-f", "segment", "-segment_time", f"{clip_length:.3f}",
        "-segment_time_delta", "0.05", "-reset_timestamps", "1", str(pattern),
    ], timeout=900)
    return sorted(p for p in pattern.parent.glob("clip_*.mp4") if p.is_file() and p.stat().st_size > 0)


def clips_have_even_durations(clips: list[Path], clip_length: float, tolerance: float = 0.25) -> bool:
    if not clips:
        return False
    durations: list[float] = []
    for clip in clips:
        try:
            durations.append(float(probe(clip)["duration"]))
        except Exception:
            return False
    # Every clip except the final one must be the requested length. The final
    # clip may be shorter, but it must never exceed the requested length.
    for duration in durations[:-1]:
        if abs(duration - clip_length) > tolerance:
            return False
    return durations[-1] <= clip_length + tolerance


def _clip_ranges(total_duration: float, clip_length: float) -> list[tuple[float, float]]:
    """Create exact ranges; never manufacture a zero-length terminal clip."""
    if total_duration <= 0:
        return []
    ...
    # If the remainder is only timestamp noise, keep the final full clip.
    if not ranges:
        ranges.append((0.0, total_duration))
    return ranges


def _write_clip_sidecars(
    segments: list[dict[str, Any]],
    start: float,
    ...
    for index, (relative_start, relative_duration) in enumerate(clip_ranges, 1):
        clip_start = start + relative_start
        clip_end = clip_start + relative_duration
        ass = job_dir / f"captions_{index:03d}.ass"
        ...
        write_caption_files(
            ...
            captions.get("position", "bottom"), ass, vtt, play_res_x, play_res_y,
        )
        # The ASS file was only needed for the full-source burn and is removed
        # here; VTT remains available for download.
...
def create_clips(job_id: str, source: Path, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> list[Path]:
    ...
    # Original/no-captions: zero-transcode first. This is the fastest possible
    # path and preserves the source codec/quality when the container permits it.
    fast_copy = export_format == "original" and not captions_enabled and abs(start) <= 0.001 and abs(end_value - duration) <= 0.2
    if fast_copy:
        copy_clips = _segment_copy(source, pattern, clip_length)
        if len(copy_clips) == expected and clips_have_even_durations(copy_clips, clip_length, tolerance=0.35):
            return copy_clips
        for old in copy_clips:
            old.unlink(missing_ok=True)

    if captions_enabled:
        # Major speed fix: transcribe once, burn captions ONCE across the full
        # requested range, then split that already-encoded result by stream copy.
        update_job(job_id, progress=10, message="Preparing captions...")
        segments = transcribe_source(source, captions.get("language", "auto"))
        relative_segments = []
        for seg in segments:
            rs = max(0.0, float(seg["start"]) - start)
            re_ = min(total_duration, float(seg["end"]) - start)
            if re_ > rs + 0.01:
                relative_segments.append({"start": rs, "end": re_, "text": seg["text"]})
        full_ass = job_dir / "captions_full.ass"
        full_vtt = job_dir / "captions_full.vtt"
        play_res_x, play_res_y = caption_canvas(export_format, info.get("width", 1920), info.get("height", 1080))
        write_caption_files(
            relative_segments, 0.0, total_duration,
            captions.get("style", "classic"), captions.get("font", "DejaVu Sans"),
            captions.get("size", "large"), captions.get("animation", "none"),
            captions.get("position", "bottom"),
            full_ass, full_vtt, play_res_x, play_res_y,
        )
        intermediate = job_dir / "captioned_source.mp4"
        try:
            if relative_segments:
                _burn_captioned_source(source, full_ass, intermediate, start, total_duration, clip_length, export_format, job_id)
                for old in clip_dir.glob("clip_*.mp4"):
                    old.unlink(missing_ok=True)
                clips = _segment_copy(intermediate, pattern, clip_length)
            else:
                # No speech: do not spend time burning an empty subtitle track.
                # Encode/segment normally once, preserving the requested format.
                filters = video_filter(export_format)
                last_boundary = max(0.0, total_duration - 0.05)
                force_expr = f"expr:if(lt(t,{last_boundary:.3f}),gte(t,n_forced*{clip_length:.3f}),0)"
                cmd = [
                    FFMPEG, "-hide_banner", "-loglevel", "error", "-y", "-ss", f"{start:.3f}", "-i", str(source),
                    "-t", f"{total_duration:.3f}", "-map", "0:v:0", "-map", "0:a?", "-c:v", "libx264",
                    "-preset", "ultrafast", "-crf", "23", "-threads", str(FFMPEG_THREADS), "-force_key_frames", force_expr,
                    "-c:a", "aac", "-b:a", "128k", "-pix_fmt", "yuv420p", "-f", "segment", "-segment_time", f"{clip_length:.3f}",
                    "-segment_time_delta", "0.05", "-reset_timestamps", "1",
                ]
                if filters:
                    cmd += ["-vf", filters]
                cmd += [str(pattern)]
                run_ffmpeg_progress(cmd, job_id, total_duration, 15, 60, max(900, int(total_duration * 4)))
                clips = sorted(p for p in clip_dir.glob("clip_*.mp4") if p.is_file() and p.stat().st_size > 0)
            if len(clips) != expected or not clips_have_even_durations(clips, clip_length, tolerance=0.35):
                raise RuntimeError("The caption/export engine could not produce evenly timed clips.")
            # Generate per-clip VTT sidecars from the single transcription.
            _write_clip_sidecars(segments, start, ranges, job_dir, captions, play_res_x, play_res_y)
            return clips
        finally:
    ...
    # Non-caption export modes: one FFmpeg encode pass, with keyframes at the
    # selected boundaries. This is the fastest accurate path when a filter is
    # required (resolution/aspect-ratio changes or a custom start/end).
    filters = video_filter(export_format)
    last_boundary = max(0.0, total_duration - 0.05)
    force_expr = f"expr:if(lt(t,{last_boundary:.3f}),gte(t,n_forced*{clip_length:.3f}),0)"
    cmd = [
        FFMPEG, "-hide_banner", "-loglevel", "error", "-y", "-ss", f"{start:.3f}", "-i", str(source),
        "-t", f"{total_duration:.3f}", "-map", "0:v:0", "-map", "0:a?", "-c:v", "libx264", "-preset", "ultrafast",
        "-crf", "21" if export_format == "original" else "23", "-threads", str(FFMPEG_THREADS),
        "-force_key_frames", force_expr, "-c:a", "aac", "-b:a", "128k", "-pix_fmt", "yuv420p",
        "-f", "segment", "-segment_time", f"{clip_length:.3f}", "-segment_time_delta", "0.05", "-reset_timestamps", "1",
    ]
    if filters:
        cmd += ["-vf", filters]
    cmd += [str(pattern)]
    run_ffmpeg_progress(cmd, job_id, total_duration, 15, 60, max(900, int(total_duration * 4)))
    clips = sorted(p for p in clip_dir.glob("clip_*.mp4") if p.is_file() and p.stat().st_size > 0)
    while len(clips) > expected:
        candidate = clips[-1]
        try:
            if probe(candidate)["duration"] < 0.5:
                candidate.unlink(missing_ok=True); clips.pop(); continue
        except Exception:
            pass
        break
    if len(clips) != expected:
        raise RuntimeError(f"Clip engine produced {len(clips)} clips; expected {expected}.")
    if not clips_have_even_durations(clips, clip_length, tolerance=0.35):
        raise RuntimeError("The clip engine could not produce evenly timed clips. Please retry with the same settings.")
    return clips
...
def complete_job(job_id: str, source: Path, source_name: str, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> None:
    try:
        ...
        job_dir = JOBS_ROOT / job_id
        # Only one thumbnail is generated synchronously. Individual clip previews
        # are generated lazily by the thumbnail endpoint, preventing N extra FFmpeg
        # processes from blocking completion—especially important for large batches.
        thumb = job_dir / "source-thumb.jpg"
        make_thumbnail(clips[0], thumb)
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        ...
        "captions_engine": "faster-whisper" if WhisperModel else "unavailable",
        "caption_model": WHISPER_MODEL_NAME,
        "max_video_size": MAX_VIDEO_SIZE,
        "max_batch_files": MAX_BATCH_FILES,
        "max_clip_seconds": MAX_CLIP_SECONDS,
        "release_status": "final-speed-accuracy-captions-batch-fix",
        "caption_runtime": "faster-whisper tiny / CPU int8",
    }
...
def run_url_import_job(
    ...
    try:
        ...
        update_job(
            ...
            status="queued",
            progress=8,
            message="Video acquired. Starting clip processing.",
        ...
        complete_job(job_id, source, filename, length, export, start, end, captions_cfg)
    except Exception as exc:
...
def source_info(url: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        "supported": False,
        "label": "Platform link",
        "message": "Platform pages are not supported. Use a direct public video-file URL.",
    }
...
async def batch(
    ...
    length, export, start_v, end_v = validate_options(clip_length, export_format, "0", "0")
    captions_cfg = captions_from_form(captions, caption_language, caption_style, caption_font, caption_size, caption_animation, caption_position)
    results = []
    for file in files:
        try:
