...
VERSION = "46.0.0-throughput-large-downloads"
...
# ZIP creation is I/O-bound and happens only after the CPU-heavy encode has
# completed. Keeping it in its own single worker prevents archive creation from
# competing with the FFmpeg worker while making large "Download all" archives
# available before a browser request has to wait for them.
zip_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="zip-worker")
...
def probe(path: Path) -> dict[str, Any]:
    result = run_checked([
        ...
        "format=duration,size:stream=index,codec_type,codec_name,width,height,codec_long_name",
        "-of", "json", str(path),
    ], timeout=60)
...
def _schedule_zip_build(job_id: str, clips: list[Path]) -> None:
    """Build the job archive after encoding, without blocking the HTTP request."""
    if not clips:
        return
    try:
        update_job(job_id, zip_status="building")
        future = zip_executor.submit(build_zip, job_id, clips)
        def finished(f):
            try:
                path = f.result()
                update_job(job_id, zip_status="ready", zip_size=path.stat().st_size if path.is_file() else 0)
                log.info("ZIP_READY job=%s size=%s", job_id, path.stat().st_size if path.is_file() else 0)
            except Exception as exc:
                log.exception("ZIP_BUILD_FAILED id=%s", job_id)
                update_job(job_id, zip_status="error", zip_error=str(exc)[:300])
        future.add_done_callback(finished)
    except Exception as exc:
        log.exception("ZIP_SCHEDULE_FAILED id=%s", job_id)
        update_job(job_id, zip_status="error", zip_error=str(exc)[:300])
...
def complete_job(job_id: str, source: Path, source_name: str, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> None:
    ...
    try:
        ...
        t = time.monotonic()
        info = probe(source)
        timings["probe_seconds"] = round(time.monotonic() - t, 3)
        ...
        timings["clipping_seconds"] = round(time.monotonic() - t, 3)
        timings["total_processing_seconds"] = round(time.monotonic() - started, 3)

        update_job(job_id, progress=92, message=f"Clips created. Finalising {len(clips)} clips...")
        job_dir = JOBS_ROOT / job_id
        thumb = job_dir / "source-thumb.jpg"
        total_duration = max(0.05, (end if end is not None else info["duration"]) - start)
        result = render_result(job_id, info, source_name, clips, thumb, None, clip_length, start, total_duration)
        result["timings"] = timings
        result["zip"] = f"/api/jobs/{job_id}/zip"
        update_job(job_id, status="complete", progress=100, message="Your clips are ready.", result=result, zip_status="building")

        # Do not put archive creation on the CPU encode critical path. The clips
        # are already complete and downloadable individually; the archive is
        # built in the background so a multi-GB job finishes processing promptly.
        _schedule_zip_build(job_id, clips)
    except Exception as exc:
        ...
        log.info("PROCESS_TIMING job=%s timings=%s", job_id, timings)
...
def download(job_id: str, name: str, request: Request):
    path = _safe_clip_path(job_id, name)
    # Use the same range-capable responder as previews. This avoids buffering
    # large clips and makes multi-hundred-MB/GB outputs resumable by clients.
    return _range_response(
        request, path, "video/mp4", "attachment", path.name
    )
...
def download_zip(job_id: str, request: Request):
    ...
    zpath = next((p for p in job_dir.glob("*.zip") if p.is_file() and p.stat().st_size > 0), None)
    if not zpath:
        clip_dir = job_dir / "clips"
        clips = sorted(
            p for p in clip_dir.glob("clip_*.mp4")
            if p.is_file() and p.stat().st_size > 0
        ) if clip_dir.is_dir() else []
        if not clips:
            raise HTTPException(404, "No completed clips were found.")

        # Normally the post-processing ZIP worker has already created this.
        # If a client requests it during that short window, build it here
        # under the same per-job lock. ZIP_STORED + Zip64 avoids compression
        # CPU and supports archives above the classic 4 GiB ZIP limit.
        zpath = build_zip(job_id, clips)

    return _range_response(
        request, zpath, "application/zip", "attachment", zpath.name
    )
