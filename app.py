...
def download_direct_media(job_id: str, url: str) -> tuple[Path, str]:
    ...
    if _host_is_youtube(urlparse(url).hostname or ""):
        ...
        if _youtube_auth_required(*errors):
            raise YouTubeAuthenticationRequired(
                "YouTube requires authentication for this video. Upload the video file instead."
            )
        raise RuntimeError(
            "We couldn't import this YouTube video automatically. Please upload the video file instead."
        )
    dest = JOBS_ROOT / job_id / "source"
    dest.mkdir(parents=True, exist_ok=True)
...
async def link(request: Request, payload: LinkRequest) -> dict[str, Any]:
    ...
    def task() -> None:
        try:
            ...
            log.info("YOUTUBE_AUTH_REQUIRED job=%s", job_id)
            update_job(
                job_id,
                status="error",
                progress=0,
                message="YouTube requires authentication for this video. Upload the video file instead.",
...
async def source_check(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if _host_is_youtube(host):
        ...
        return {"ok": True, "kind": "youtube", "message": "YouTube link accepted. ClipShortener will try automatic acquisition first; if authentication is required, you can upload the video file instead."}
    if _host_is_platform(host):
