...
VERSION = "34.0.0-stable-reliability-speed"
...
UPLOAD_CHUNK_SIZE = int(os.getenv("CLIPSHORTENER_UPLOAD_CHUNK_MB", "128")) * 1024 * 1024
MAX_BATCH_FILES = 20
...
def create_clips(job_id: str, source: Path, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> list[Path]:
    ...
    copy_cmd += ["-i", str(source), "-t", f"{total_duration:.3f}", "-map", "0:v:0", "-map", "0:a?", "-c", "copy", "-avoid_negative_ts", "make_zero", "-f", "segment", "-segment_time", f"{clip_length:.6f}", "-reset_timestamps", "1", str(pattern)]
...
def complete_job(job_id: str, source: Path, source_name: str, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> None:
    try:
        ...
        # Clip thumbnails are generated lazily by the thumbnail endpoint so
        # preview generation never blocks completion of a large batch.
        zip_path = build_zip(job_id, clips)
        result = render_result(job_id, info, source_name, clips, thumb, zip_path, clip_length, start, max(0.05, (end if end is not None else info["duration"]) - start))
