...
VERSION = "35.0.0-speed-optimized"
...
def create_clips(job_id: str, source: Path, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> list[Path]:
    ...
    copy_allowed = export_format == "original" and not captions
    if copy_allowed:
        try:
    ...
    cmd = [
        ...
        "-map", "0:v:0", "-map", "0:a?", "-c:v", "libx264", "-preset", preset, "-crf", "21", "-threads", str(FFMPEG_THREADS),
