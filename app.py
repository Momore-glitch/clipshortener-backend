...
def _is_vimeo_url(url: str) -> bool:
    ...
    return host in {"vimeo.com", "www.vimeo.com", "player.vimeo.com"} or host.endswith(".vimeo.com")


def _vimeo_video_id(url: str) -> str:
    parsed = urlparse(url)
    if not _is_vimeo_url(url):
        raise RuntimeError("This is not a Vimeo video URL.")
    match = re.search(r"/(?:video/)?(\d+)(?:[/?#]|$)", parsed.path)
    if not match:
        match = re.search(r"(?:^|/)(\d+)(?:[/?#]|$)", parsed.path)
    if not match:
        raise RuntimeError("Could not determine the Vimeo video ID from this URL.")
    return match.group(1)


def _vimeo_api(video_id: str) -> dict[str, Any]:
    if not VIMEO_ACCESS_TOKEN:
        raise RuntimeError(
            "Vimeo acquisition is not configured. Add VIMEO_ACCESS_TOKEN to the Render environment."
        )
    request = URLRequest(
        f"{VIMEO_API_URL}/videos/{video_id}?fields=download,name,link,privacy.download",
        headers={
            "Authorization": f"bearer {VIMEO_ACCESS_TOKEN}",
            "Accept": "application/json",
            "User-Agent": "ClipShortener/29.0",
        },
    )
    try:
        ...
        try:
            ...
            exc.close()
        raise RuntimeError(f"Vimeo API returned HTTP {exc.code}. {detail[:240]}") from exc
    except (URLError, TimeoutError, OSError) as exc:
        raise RuntimeError("Could not reach the Vimeo API.") from exc
    try:
        data = json.loads(raw.decode("utf-8"))
    except Exception as exc:
        raise RuntimeError("Vimeo returned invalid API JSON.") from exc
    if not isinstance(data, dict):
        raise RuntimeError("Vimeo returned an invalid API response.")
    return data


def _acquire_vimeo(job_id: str, url: str) -> tuple[Path, str]:
    video_id = _vimeo_video_id(url)
    data = _vimeo_api(video_id)
    downloads = data.get("download") or []
    if not isinstance(downloads, list) or not downloads:
        raise RuntimeError(
            "Vimeo did not provide a downloadable file for this video. "
            "The authenticated Vimeo account must have download permission."
        )
    ...
    if not candidates:
        raise RuntimeError("Vimeo provided no usable downloadable file.")

    # Prefer the largest available authorized file.
    def download_score(item: dict[str, Any]) -> tuple[int, int, int]:
        try:
    ...
    selected = max(candidates, key=download_score)
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.hostname or not _host_is_public(parsed.hostname):
        raise RuntimeError("Vimeo returned an unsafe download URL.")

    title = safe_filename(str(data.get("name") or f"vimeo-{video_id}"))
    suffix = Path(urlparse(download_url).path).suffix.lower()
    if suffix not in SUPPORTED_EXTENSIONS:
        suffix = ".mp4"
    if not title.lower().endswith(suffix):
        title = Path(title).stem + suffix
    ...
    try:
        with urlopen(req, timeout=ACQUISITION_TIMEOUT) as response:
            ...
            if content_length:
                try:
                    if int(content_length) > MAX_VIDEO_SIZE:
                        raise RuntimeError("The Vimeo video exceeds the configured upload limit.")
                except ValueError:
                    pass
            with source.open("wb") as target:
                while True:
        ...
        source.unlink(missing_ok=True)
        raise RuntimeError(f"The Vimeo download returned HTTP {exc.code}.") from exc
    except (URLError, TimeoutError, OSError) as exc:
        source.unlink(missing_ok=True)
        raise RuntimeError("Could not download the authorized Vimeo file.") from exc

    if total <= 0:
        source.unlink(missing_ok=True)
        raise RuntimeError("Vimeo returned an empty video file.")

    log.info("VIMEO_IMPORT_SUCCESS job=%s video_id=%s size=%s", job_id, video_id, total)
    return source, title
...
def _acquire_video(job_id: str, url: str) -> tuple[Path, str]:
    ...
    if is_platform:
        if _is_vimeo_url(value):
            return _acquire_vimeo(job_id, value)
        raise RuntimeError(
            "This platform link is not enabled. ClipShortener supports direct public media URLs "
            "and authorized Vimeo downloads in this phase."
        )
...
def validate_options(clip_length: Any, export_format: str, start: Any, end: Any) -> tuple[float, str, float, float | None]:
    ...
    allowed_exports = {"original", "720p", "1080p", "vertical", "square", "landscape"}
...
def _caption_segments(video: Path, language: str) -> list[tuple[float, float, str]]:
...
def caption_tracks_from_segments(
    ...
    def ass_time(seconds: float) -> str:
        ...
        h, cs = divmod(cs, 360000); m, cs = divmod(cs, 6000); sec, cs = divmod(cs, 100)
    ...
    with ass.open("w", encoding="utf-8") as af, vtt.open("w", encoding="utf-8") as vfout:
        ...
        for start, end, text in segments:
            ...
            if local_end <= local_start:
                continue
            af.write(f"Dialogue: 0,{ass_time(local_start)},{ass_time(local_end)},Default,,0,0,0,,{{{settings['tags']}}}{ass_escape(text)}\n")
            vfout.write(f"{vtt_time(local_start)} --> {vtt_time(local_end)}\n{text}\n\n")
            count += 1
    if count == 0:
...
def burn_captions(video: Path, ass_path: Path, output: Path, job_id: str | None = None, duration: float | None = None) -> None:
    ...
    if job_id and duration:
        run_ffmpeg_progress(cmd, job_id, duration, 70, 20, max(900, int(duration * 8)))
    else:
        run_checked(cmd, timeout=1800)


def _durations_are_consistent(clips: list[Path], clip_length: float, total_duration: float, tolerance: float = 0.20) -> bool:
    """Validate count/non-empty outputs and sample timing without spawning an
    ffprobe process for every clip. This matters for long multi-GB sources."""
    if not clips:
        return False
    ...
    # Check first, middle and last outputs. The segment muxer creates every
    # intermediate boundary from the same timing rule, so this catches the
    # common keyframe/timestamp failures without turning a 2 GB job into
    # hundreds of serial ffprobe launches.
...
def export_matches_source(info: dict[str, Any], export_format: str) -> bool:
    """Return True when an export option requires no video transformation."""
    ...
    target = targets.get(export_format)
    return target is not None and (width, height) == target

def create_clips(job_id: str, source: Path, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None, info: dict[str, Any] | None = None) -> list[Path]:
    ...
    # Fast path: no video transformation and no captions. Stream copy avoids
    # decoding/re-encoding the source and is the only practical path toward
    # multi-gigabyte processing in minutes on the current CPU tier.
    copy_cmd = [
        FFMPEG, "-hide_banner", "-loglevel", "error", "-y",
    ]
    if start > 0.001:
        copy_cmd += ["-ss", f"{start:.3f}"]
    copy_cmd += ["-i", str(source), "-t", f"{total_duration:.3f}", "-map", "0:v:0", "-map", "0:a?", "-c", "copy", "-avoid_negative_ts", "make_zero", "-f", "segment", "-segment_time", f"{clip_length:.6f}", "-segment_time_delta", "0.02", "-break_non_keyframes", "1", "-reset_timestamps", "1", str(pattern)]

    copy_allowed = (export_format == "original" or export_matches_source(info, export_format)) and not captions
    if copy_allowed:
        try:
            ...
            if not _durations_are_consistent(copy_clips, clip_length, total_duration):
                for old in copy_clips: old.unlink(missing_ok=True)
            else:
                return copy_clips
        except Exception:
            for old in clip_dir.glob("clip_*.mp4"): old.unlink(missing_ok=True)

    # Accurate fallback: decode once and force keyframes at the requested clip
    # boundaries. This is slower but provides deterministic segment lengths.
    filters = video_filter(export_format)
    preset = "ultrafast"
    cmd = [
        FFMPEG, "-hide_banner", "-loglevel", "error", "-y", "-sws_flags", "fast_bilinear", "-ss", f"{start:.3f}", "-i", str(source), "-t", f"{total_duration:.3f}",
        "-map", "0:v:0", "-map", "0:a?", "-c:v", "libx264", "-preset", preset, "-tune", "zerolatency", "-crf", "23", "-threads", str(FFMPEG_THREADS),
        "-force_key_frames", f"expr:gte(t,n_forced*{clip_length:.6f})", "-c:a", "aac", "-b:a", "128k", "-f", "segment",
        "-segment_time", f"{clip_length:.6f}", "-segment_time_delta", "0.02", "-reset_timestamps", "1",
    ]
    if filters: cmd += ["-vf", filters]
    cmd += [str(pattern)]
    run_ffmpeg_progress(cmd, job_id, total_duration, 20, 50, max(900, int(total_duration * 6)))

    clips = sorted(p for p in clip_dir.glob("clip_*.mp4") if p.is_file() and p.stat().st_size > 0)
    if len(clips) > expected:
        extras = clips[expected:]
        for extra in extras: extra.unlink(missing_ok=True)
        clips = clips[:expected]
    if not clips:
        raise RuntimeError("No clips were produced.")
    if not _durations_are_consistent(clips, clip_length, total_duration, tolerance=0.30):
        raise RuntimeError("Clip timing could not be made consistent for this source. No partial result was returned.")

    if captions and captions.get("enabled"):
        segments = _caption_segments(source, captions.get("language", "auto"))
        for index, clip in enumerate(clips, 1):
            clip_start = start + (index - 1) * clip_length
            clip_duration = probe(clip)["duration"]
            ass = job_dir / f"captions_{index:03d}.ass"
            vtt = job_dir / f"captions_{index:03d}.vtt"
            caption_tracks_from_segments(
                segments, clip_start, clip_duration,
                captions.get("style", "classic"), captions.get("font", "DejaVu Sans"),
                captions.get("size", "large"), captions.get("animation", "none"),
                captions.get("position", "bottom"), ass, vtt,
            )
            if ass.exists():
...
def render_result(job_id: str, source_info: dict[str, Any], source_name: str, clips: list[Path], thumb: Path, zip_path: Path, clip_length: float, start: float, total_duration: float) -> dict[str, Any]:
    items = []
    expected_last = total_duration - clip_length * (len(clips) - 1)
    if expected_last <= 0.35 and len(clips) > 1:
        expected_last = clip_length
    for index, clip in enumerate(clips):
        duration = clip_length if index < len(clips) - 1 else expected_last
...
def complete_job(job_id: str, source: Path, source_name: str, clip_length: float, export_format: str, start: float, end: float | None, captions: dict[str, Any] | None) -> None:
    try:
        ...
        # Source thumbnails and ZIP archives are deliberately lazy. Building a
        # ZIP immediately after clipping a multi-GB source would duplicate all
        # output bytes before the user can download anything.
        job_dir = JOBS_ROOT / job_id
        thumb = job_dir / "source-thumb.jpg"
        result = render_result(job_id, info, source_name, clips, thumb, None, clip_length, start, max(0.05, (end if end is not None else info["duration"]) - start))
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        ...
        "input_mode": "uploads + direct public media + authorized Vimeo downloads",
        "url_acquisition": "direct-media service" if ACQUISITION_API_URL else "local_fallback",
        "vimeo_acquisition": "configured" if VIMEO_ACCESS_TOKEN else "not_configured",
...
def source_info(url: str = "") -> dict[str, Any]:
    ...
    if _is_vimeo_url(value):
        configured = bool(VIMEO_ACCESS_TOKEN)
        return {
            "kind": "vimeo",
            "supported": configured,
            "label": "Vimeo",
            "message": (
                "Authorized Vimeo acquisition is ready."
                if configured else
                "Vimeo integration needs VIMEO_ACCESS_TOKEN on Render."
            ),
        }
    return {
        "kind": "platform",
...
async def batch(
    ...
    results = []
    request.state.batch_upload = True
    for file in files:
        try:
...
def download_zip(job_id: str) -> FileResponse:
    ...
    if not job_dir.is_dir():
        raise HTTPException(404, "Job not found or expired.")
    zpath = next((p for p in job_dir.glob("*.zip") if p.is_file()), None)
    if not zpath:
        clip_dir = job_dir / "clips"
        clips = sorted(p for p in clip_dir.glob("clip_*.mp4") if p.is_file() and p.stat().st_size > 0) if clip_dir.is_dir() else []
        if not clips:
            raise HTTPException(404, "No completed clips were found.")
        zpath = build_zip(job_id, clips)
