...
VERSION = "38.0.0-reliability-speed"
...
JOB_TTL = 60 * 60 * 6
MAX_WORKERS = max(1, min(2, int(os.getenv("CLIPSHORTENER_WORKERS", "2"))))
RATE_LIMIT = 20
RATE_WINDOW = 600
...
FFMPEG_THREADS = max(1, min(2, int(os.getenv("CLIPSHORTENER_FFMPEG_THREADS", "1"))))
...
def runtime_diagnostics() -> dict[str, Any]:
    return {
        ...
        "queue_model": "FIFO bounded processing with controlled concurrent workers; first queued jobs start first",
...
def _range_response(request: Request, path: Path, media_type: str, download_name: str | None = None):
    ...
    if not range_header:
        ...
        return FileResponse(path, media_type=media_type, filename=None, headers=headers)
...
def download(job_id: str, name: str, request: Request):
    ...
    return _range_response(request, path, "video/mp4", path.name)
...
def download_zip(job_id: str, request: Request):
    ...
    return _range_response(request, zpath, "application/zip", zpath.name)
