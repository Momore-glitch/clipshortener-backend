...
FFMPEG_FILTER_THREADS = max(0, min(8, int(os.getenv("CLIPSHORTENER_FILTER_THREADS", "0"))))
...
def create_clips(job_id, source, clip_length, export_format, start, end, captions, info=None):
    ...
    # ---------------------------------------------------------------
    # Encode the transformed video ONCE, then segment the encoded file with
    # stream-copy. This keeps the CPU-heavy encode completely separate from
    # MP4 segment muxing and avoids making libx264 repeatedly deal with the
    # segment muxer while it is encoding.
    last_boundary = max(0.0, total_duration - 0.05)
    force_expr = f"expr:if(lt(t,{last_boundary:.3f}),gte(t,n_forced*{clip_length:.6f}),0)"
    encoded = job_dir / "encoded.mp4"

    # A single continuous MP4 encode is the hot path. The segment files are
    # produced afterwards using -c copy, so no second video encode occurs.
    encode_cmd = [
        FFMPEG, "-hide_banner", "-loglevel", "error", "-y",
        "-sws_flags", "fast_bilinear",
        "-ss", f"{start:.3f}", "-i", str(source),
    ...
    run_ffmpeg_progress(
        encode_cmd, job_id, total_duration, 20, 45,
        max(900, int(total_duration * 6)),
    )

    # Segmenting an already encoded file is intentionally outside the FFmpeg
    # CPU slot's expensive encode work. It is stream-copy only and therefore
    # normally completes in a small fraction of realtime.
    segment_cmd = [
    ...
    run_checked(segment_cmd, timeout=max(300, int(total_duration * 2) + 120))
